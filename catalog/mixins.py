...
class ProductCreateUpdateMixin(View):
    formset_class = inlineformset_factory(Product, Gallery, form=GalleryForm, fields=('image',), min_num=2, extra=1)
    ...
    def form_valid(self, product):
        formset = self.formset_class(self.request.POST, self.request.FILES, instance=product)
        if formset.is_valid():
            formset.save()
    ...
    def post(self, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            return render(self.request, 'catalog/product_create.html', {'product_creation_errors': form.errors})
